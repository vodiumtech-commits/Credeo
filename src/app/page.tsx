    ...
    <main className="bg-vodium-black text-vodium-cream overflow-x-hidden">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 glass-dark border-b border-white/[0.06]">
        <div className="max-w-6xl mx-auto px-6 md:px-12 flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm text-vodium-cream/55">
            <a href="#how-it-works" className="hover:text-vodium-gold transition-colors duration-150">How it works</a>
            <a href="#features"     className="hover:text-vodium-gold transition-colors duration-150">Features</a>
            <a href="#pricing"      className="hover:text-vodium-gold transition-colors duration-150">Pricing</a>
          </div>

          {/* Desktop CTAs */}
          <div className="hidden md:flex items-center gap-3">
            <Link href="/login" className="text-sm text-vodium-cream/55 hover:text-vodium-gold transition-colors duration-150">
              Sign in
            </Link>
            <Link href="/register" className="btn-gold px-5 py-2.5 rounded-lg text-sm">
              Get started free
            </Link>
          </div>
          ...
          {mobileOpen && (
            ...
            >
              <div className="px-6 py-5 flex flex-col gap-4 text-sm">
                <a href="#how-it-works" onClick={() => setMobileOpen(false)} className="text-vodium-cream/60 hover:text-vodium-gold transition-colors">How it works</a>
                <a href="#features"     onClick={() => setMobileOpen(false)} className="text-vodium-cream/60 hover:text-vodium-gold transition-colors">Features</a>
                <a href="#pricing"      onClick={() => setMobileOpen(false)} className="text-vodium-cream/60 hover:text-vodium-gold transition-colors">Pricing</a>
                <div className="border-t border-white/[0.07] pt-4 flex flex-col gap-3">
                  <Link href="/login" className="text-vodium-cream/60 hover:text-vodium-gold transition-colors">Sign in</Link>
                  <Link href="/register" className="btn-gold px-5 py-3 rounded-lg text-sm text-center">Get started free</Link>
                </div>
              </div>
            </motion.div>
          )}
      ...
      <section className="relative min-h-screen flex items-center mesh-bg bg-dots overflow-hidden pt-16">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 py-24 md:py-36">
          ...
          >
            Vodium Ledger is the WhatsApp-first credit ledger built for Nigerian campus vendors.
            Record a credit in 15 seconds. Auto-remind students. Track every naira you&rsquo;re owed —
            from your phone, not an app.
          </motion.p>

          <motion.div {...fadeUp(0.28)} className="flex flex-col sm:flex-row gap-4 mb-16">
            <Link href="/register">
              <ShimmerButton className="h-13 px-8 gap-2 text-base">
          ...
          >
            {[
              ...
              >
                <p className="font-serif text-xl md:text-2xl text-vodium-gold leading-none mb-1">{s.value}</p>
                <p className="text-vodium-cream/45 text-[11px] uppercase tracking-wider">{s.label}</p>
              </div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={s.label}>
                <p className="font-serif text-3xl md:text-4xl text-vodium-gold mb-1">{s.value}</p>
      ...
      <section id="how-it-works" className="py-24 md:py-32 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp()}>
            <p className="text-vodium-gold text-xs tracking-[0.35em] uppercase mb-4">Simple by design</p>
              ...
              <motion.div key={step.number} {...fadeUp(i * 0.1)}>
                <AnimatedBorder className="p-7 h-full">
                  <div className="flex items-start justify-between mb-6">
                    <p className="font-serif text-5xl text-vodium-gold/20 leading-none">{step.number}</p>
                  ...
                  <h3 className="font-serif text-2xl mb-3 text-vodium-cream">{step.title}</h3>
                  <p className="text-vodium-cream/55 leading-relaxed text-sm">{step.body}</p>
                </AnimatedBorder>
              </motion.div>
            ))}
          </div>
      ...
      <section id="features" className="py-24 md:py-32 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp()}>
            <p className="text-vodium-gold text-xs tracking-[0.35em] uppercase mb-4">Everything you need</p>
            ...
            {[
              ...
              <motion.div key={f.title} {...fadeUp(f.delay)}>
                <MagicCard className="p-7 h-full">
                  ...
                  <h3 className="font-serif text-xl mb-3 text-vodium-cream">{f.title}</h3>
                  <p className="text-vodium-cream/50 leading-relaxed text-sm">{f.body}</p>
                </MagicCard>
              </motion.div>
            ))}
          </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-2">
                {["M", "T", "B", "W", "A"].map((initial) => (
                  ...
                  >
                    <span className="font-serif text-vodium-gold text-xs">{initial}</span>
                  </div>
                ))}
              </div>
              <div>
                <p className="text-vodium-cream text-sm font-medium">127 campus vendors trust Vodium Ledger</p>
                <div className="flex items-center gap-1 mt-0.5">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={11} className="text-vodium-gold fill-vodium-gold" />
                  ))}
                  <span className="text-vodium-cream/40 text-xs ml-1">4.9 / 5 from pilot vendors</span>
                </div>
              </div>
            </div>
      ...
      <section className="py-24 md:py-32 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp()}>
            <p className="text-vodium-gold text-xs tracking-[0.35em] uppercase mb-4 text-center">What vendors say</p>
            <h2 className="font-serif text-4xl md:text-5xl mb-14 text-center">Real results.<br />Real vendors.</h2>
          </motion.div>
              ...
              <motion.div key={t.name} {...fadeUp(t.delay)}>
                <AnimatedBorder className="p-8 h-full">
                  <div className="flex gap-1 mb-5">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={14} className="text-vodium-gold fill-vodium-gold" />
                    ))}
                  </div>
                  ...
                  <div>
                    <p className="font-medium text-vodium-cream text-sm">{t.name}</p>
                    <p className="text-vodium-cream/40 text-xs mt-0.5">{t.role}</p>
                  </div>
                </AnimatedBorder>
              </motion.div>
            ))}
      ...
      <section id="pricing" className="py-24 md:py-32 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp()}>
            <p className="text-vodium-gold text-xs tracking-[0.35em] uppercase mb-4 text-center">Pricing</p>
            ...
            <p className="text-vodium-cream/55 text-center mb-14 max-w-lg mx-auto">
              All plans include 60 days free. No credit card required. No hidden fees. Cancel any time.
            </p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={tier.plan} {...fadeUp(tier.delay)} className="relative">
                {tier.popular && (
                ...
                >
                  <p className="text-vodium-cream/45 text-xs font-semibold tracking-widest uppercase mb-4">{tier.plan}</p>
                  <p className="font-serif text-4xl text-vodium-cream mb-0.5">{tier.price}</p>
                  <p className="text-xs text-vodium-cream/35 mb-4">{tier.sub}</p>
                  <p className="text-vodium-cream/60 text-sm mb-7 leading-relaxed">{tier.description}</p>
                  <ul className="space-y-3 mb-8 flex-1">
                    {tier.features.map((f) => (
                      <li key={f} className="flex items-start gap-2.5 text-sm text-vodium-cream/70">
                        <CheckCircle size={14} className="text-vodium-gold flex-shrink-0 mt-0.5" />
                        {f}
                      </li>
                    ))}
                  </ul>
      ...
      <section className="mesh-bg py-24 md:py-36 px-6 md:px-12 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          ...
          <motion.h2 {...fadeUp(0.08)} className="font-serif text-4xl md:text-6xl mb-6 leading-tight">
            Built for the vendor.<br />
            <em className="text-gradient-gold not-italic">Owned by Africa.</em>
          </motion.h2>
          <motion.p {...fadeUp(0.16)} className="text-vodium-cream/55 mb-10 text-lg leading-relaxed">
            Vodium Ledger is the first step in Africa&rsquo;s premium credit ecosystem.
            Every credit you track builds the graph that makes lending smarter for everyone.
          </motion.p>
          <motion.div {...fadeUp(0.22)}>
            <Link href="/register">
              <ShimmerButton className="h-14 px-10 gap-2 text-base">
          ...
          <div className="grid md:grid-cols-4 gap-10 mb-12">
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="font-serif tracking-[0.15em] text-vodium-gold text-xs">VODIUM LEDGER</span>
              </div>
              <p className="text-vodium-cream/35 text-sm leading-relaxed">
                Africa&rsquo;s credit infrastructure layer. Starting with Nigerian university campus vendors.
              </p>
            </div>
