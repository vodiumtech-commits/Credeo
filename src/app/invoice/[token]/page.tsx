...
export default async function PublicInvoicePage({ params }: { params: { token: string } }) {
  ...
  return (
    <div className="min-h-dvh bg-[#0A0A0A] text-vodium-cream px-5 py-10 flex justify-center">
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-vodium-charcoal overflow-hidden">
          {/* Head */}
          <div className="p-6 border-b border-white/[0.06]" style={{ background: `linear-gradient(140deg, ${brand}14, transparent)` }}>
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] inline-flex items-center gap-1.5" style={{ color: brand }}>
                  <FileText size={13} /> Invoice
                </p>
                <h1 className="font-serif text-2xl mt-2">{formatNaira(total)}</h1>
                <p className="text-xs text-vodium-cream/40 mt-1">{invoice.invoiceNumber}</p>
              </div>
              <span
          ...
          <div className="p-6 space-y-3">
            ...
            <div className="pt-3 mt-3 border-t border-white/[0.06] space-y-1.5 text-sm">
              ...
              <div className="flex justify-between font-semibold pt-1.5">
                <span>Total</span>
                <span style={{ color: brand }}>{formatNaira(total)}</span>
              </div>
              {paid > 0 && (
                <>
                  ...
                  <div className="flex justify-between font-semibold">
                    ...
                    <span style={{ color: brand }}>{formatNaira(outstanding)}</span>
                  </div>
                </>
              )}
            </div>
        ...
        <p className="text-center text-[11px] text-vodium-cream/25 mt-5">Powered by Vodium Ledger</p>
      </div>
    </div>
  );
}
