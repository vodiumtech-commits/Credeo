...
import { Shield, Eye, EyeOff, Lock, CheckCircle2, Loader2, AlertTriangle } from "lucide-react";
...
export default function AdminInvitePage() {
  ...
  if (status !== "valid") {
    const messages: Record<Exclude<Status, "loading" | "valid">, { title: string; body: string }> = {
      invalid:  { title: "Invalid link",       body: "This invitation link is invalid or doesn't exist." },
      expired:  { title: "Link expired",       body: "This invitation has expired. Ask Super Admin to resend your invite." },
      used:     { title: "Already activated",  body: "This invitation has already been used. Go to the login page." },
    };
    ...
    return (
      <div className="min-h-screen bg-vodium-black flex items-center justify-center px-6">
        <div className="bg-vodium-charcoal border border-white/[0.08] rounded-2xl p-10 max-w-md w-full text-center">
          ...
          <h1 className="font-serif text-xl text-vodium-cream mb-2">{msg.title}</h1>
          <p className="text-vodium-cream/45 text-sm mb-6">{msg.body}</p>
          <a href="/admin/login" className="btn-gold px-6 py-2.5 rounded-xl text-sm inline-block">
            Go to login
          </a>
        </div>
      </div>
      ...
      <div className="w-full max-w-md">
        <div className="bg-vodium-charcoal border border-white/[0.08] rounded-2xl p-10 shadow-[0_0_60px_rgba(201,169,97,0.06)]">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-vodium-gold text-xs tracking-[0.3em] uppercase mb-1.5">Admin Console</p>
            <h1 className="font-serif text-2xl text-vodium-cream text-center">Activate your account</h1>
          ...
          <div className="bg-vodium-gold/[0.06] border border-vodium-gold/15 rounded-xl px-5 py-4 mb-7 flex items-center gap-3">
            <CheckCircle2 size={16} className="text-vodium-gold flex-shrink-0" />
            <div>
              <p className="text-xs text-vodium-cream/50 uppercase tracking-wider">Your role</p>
              <p className="text-vodium-cream font-semibold text-sm mt-0.5">
                {ROLE_LABELS[info!.role] ?? info!.role}
              </p>
            </div>
            <p className="text-xs text-vodium-cream/30 ml-auto">{info!.email}</p>
          </div>
          ...
          <form onSubmit={handleActivate} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <Lock size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-vodium-cream/30 pointer-events-none" />
                <input
            ...
            >
              {loading ? <><Loader2 size={14} className="animate-spin" /> Activating…</> : "Activate account & sign in"}
            </button>
          </form>
        </div>
      </div>
