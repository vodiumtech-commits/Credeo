...
export function AdminShell({ name, role, children }: Omit<Props, 'email'>) {
  ...
  function NavItem({ href, icon: Icon, label, mobile = false }: {
    href: string; icon: React.ElementType; label: string; mobile?: boolean;
  }) {
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D] flex">
      ...
      <aside className="hidden md:flex flex-col bg-[#080808] border-r border-white/[0.05] fixed inset-y-0 left-0 z-30 w-14 xl:w-[220px] transition-all duration-300">
        ...
        <div className="h-14 flex items-center justify-center xl:justify-start xl:px-5 border-b border-white/[0.05] flex-shrink-0">
          ...
          <div className="hidden xl:block ml-3 overflow-hidden">
            <p className="font-serif text-[11px] tracking-[0.22em] text-vodium-gold leading-none whitespace-nowrap">VODIUM</p>
            <p className="text-[9px] text-vodium-cream/20 tracking-wider mt-1 whitespace-nowrap font-medium">ADMIN CONSOLE</p>
          </div>
        </div>

        {/* Nav group */}
        <div className="px-2 pt-5 pb-1">
          <p className="hidden xl:block text-[9px] font-bold tracking-[0.18em] text-vodium-cream/20 uppercase px-2 mb-2.5">Navigation</p>
        </div>
        ...
        <div className="hidden xl:flex items-center gap-2.5 px-4 py-2 mx-2 mb-2 rounded-lg bg-emerald-500/[0.06] border border-emerald-500/15">
          ...
          <span className="text-[10px] text-emerald-400/70 font-medium">All systems live</span>
        </div>
        ...
        <div className="flex flex-col items-center xl:items-stretch gap-0.5 px-2 pb-4 border-t border-white/[0.05] pt-3">
          <div className="group relative flex items-center justify-center xl:justify-start h-10 xl:h-auto xl:px-3 xl:py-2.5 rounded-xl border border-transparent">
            ...
            <div className="hidden xl:block ml-2.5 overflow-hidden min-w-0">
              <p className="text-vodium-cream text-[12px] font-semibold leading-none whitespace-nowrap truncate max-w-[120px]">{name}</p>
              <p className={`text-[10px] mt-1 font-bold px-1.5 py-0.5 rounded-md border inline-block ${roleColor}`}>{roleLabel}</p>
            </div>
      ...
      {/* ── Mobile overlay ─────────────────────────────────────────────────── */}
      {open && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-40 md:hidden" onClick={() => setOpen(false)} />
      )}
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#080808] flex flex-col border-r border-white/[0.05] md:hidden transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="h-14 px-5 flex items-center justify-between border-b border-white/[0.05]">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-xl bg-vodium-charcoal border border-vodium-gold/30 flex items-center justify-center">
              <span className="font-serif text-vodium-gold text-xs leading-none">V</span>
            </div>
            <div>
          ...
          <button onClick={() => setOpen(false)} className="w-7 h-7 rounded-lg text-vodium-cream/40 hover:text-vodium-cream hover:bg-white/[0.06] flex items-center justify-center transition-colors">
            <X size={16} />
          </button>
        </div>
        <div className="px-3 pt-5 pb-2">
          <p className="text-[9px] font-bold tracking-[0.18em] text-vodium-cream/20 uppercase px-3 mb-2">Navigation</p>
        </div>
        <nav className="flex-1 px-3 space-y-0.5">
          {NAV.map((item) => <NavItem key={item.href} {...item} mobile />)}
        </nav>
        <div className="px-3 pb-5 border-t border-white/[0.05] pt-4 space-y-1">
          <div className="flex items-center gap-2.5 px-3 py-2.5">
            ...
            <div>
              <p className="text-vodium-cream text-xs font-semibold leading-none">{name}</p>
              <p className={`text-[10px] mt-1 font-bold px-1.5 py-0.5 rounded-md border inline-block ${roleColor}`}>{roleLabel}</p>
            </div>
          </div>
          <button onClick={handleSignOut} className="flex items-center gap-3 px-3 py-2.5 rounded-xl w-full text-sm text-vodium-cream/35 hover:text-rose-400 hover:bg-rose-500/[0.06] transition-all border border-transparent">
      ...
      <div className="flex-1 md:ml-14 xl:ml-[220px] flex flex-col min-h-screen transition-all duration-300">
        ...
        <header className="bg-[#0A0A0A]/95 backdrop-blur-xl border-b border-white/[0.05] sticky top-0 z-20 px-5 md:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => setOpen(true)} className="md:hidden w-8 h-8 rounded-lg text-vodium-cream/50 hover:text-vodium-cream hover:bg-white/[0.06] flex items-center justify-center transition-colors">
              <Menu size={17} />
            </button>
            {/* Breadcrumb */}
            <nav className="hidden md:flex items-center gap-1.5 text-[11px]">
              <Link href="/admin" className="text-vodium-cream/25 hover:text-vodium-gold transition-colors flex items-center gap-1">
                <Shield size={11} className="text-vodium-gold/40" /> Admin Console
              </Link>
              {breadcrumb !== "Overview" && (
                <>
                  <ChevronRight size={10} className="text-vodium-cream/15" />
                  <span className="text-vodium-cream/70 font-medium">{breadcrumb}</span>
                </>
              )}
            </nav>
            <p className="md:hidden text-sm font-semibold text-vodium-cream/80">{breadcrumb}</p>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            {/* Role badge */}
            <span className={`hidden md:inline-flex items-center text-[10px] font-bold px-2.5 py-1 rounded-lg border ${roleColor}`}>
              {roleLabel}
            </span>
            {/* Bell */}
