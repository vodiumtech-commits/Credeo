...
const STATUS_CONFIG: Record<CreditStatus, { label: string; className: string; dot: string }> = {
  OUTSTANDING:    { label: "Outstanding",   className: "text-sky-400 bg-sky-500/10 border border-sky-500/20",              dot: "bg-sky-400" },
  DUE_SOON:       { label: "Due soon",      className: "text-amber-400 bg-amber-500/10 border border-amber-500/20",        dot: "bg-amber-400" },
  OVERDUE:        { label: "Overdue",       className: "text-rose-400 bg-rose-500/10 border border-rose-500/20",           dot: "bg-rose-400" },
  PARTIALLY_PAID: { label: "Partial",       className: "text-purple-400 bg-purple-500/10 border border-purple-500/20",    dot: "bg-purple-400" },
  PAID:           { label: "Paid",          className: "text-emerald-400 bg-emerald-500/10 border border-emerald-500/20", dot: "bg-emerald-400" },
  WRITTEN_OFF:    { label: "Written off",   className: "text-vodium-cream/30 bg-white/[0.04] border border-white/[0.07]", dot: "bg-vodium-cream/20" },
};
...
export function CreditsClient({
  ...
  const statCards = [
    { label: "Outstanding", value: formatNaira(stats.outstandingAmount), icon: <DollarSign size={16} />,  accent: "text-sky-400",     ring: "ring-sky-500/20",     bg: "bg-sky-500/[0.07]" },
    { label: "Overdue",     value: formatNaira(stats.overdueAmount),     icon: <AlertCircle size={16} />, accent: "text-rose-400",     ring: "ring-rose-500/20",    bg: "bg-rose-500/[0.07]" },
    { label: "Recovered",   value: formatNaira(stats.paidAmount),        icon: <CheckCircle2 size={16} />,accent: "text-emerald-400",  ring: "ring-emerald-500/20", bg: "bg-emerald-500/[0.07]" },
    { label: "Recovery",    value: `${stats.recoveryRate}%`,             icon: <TrendingUp size={16} />,  accent: "text-vodium-gold",  ring: "ring-vodium-gold/20", bg: "bg-vodium-gold/[0.07]" },
  ] as const;
    ...
    <div className="p-5 md:p-8 max-w-[1200px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statCards.map((s) => (
          ...
          >
            ...
            <p className={`font-serif text-xl ${s.accent} leading-none`}>{s.value}</p>
            <p className="text-[10px] text-vodium-cream/35 uppercase tracking-widest font-semibold">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          ...
          >
            <ArrowUpDown size={13} />
            Sort: {sortBy === "date" ? "Date added" : sortBy === "amount" ? "Amount" : "Due date"}
            <ChevronDown size={11} />
          </button>
          {showSort && (
            <div className="absolute right-0 top-full mt-1.5 bg-vodium-charcoal border border-white/[0.09] rounded-xl shadow-xl z-20 min-w-[160px] py-1 overflow-hidden">
              {(["date", "amount", "due"] as const).map((k) => (
                ...
                >
                  {k === "date" ? "Date added" : k === "amount" ? "Amount" : "Due date"}
                  {sortBy === k && <span className="ml-1.5 opacity-60">{sortDesc ? "↓" : "↑"}</span>}
                </button>
              ))}
            </div>
          )}
        ...
        {FILTER_TABS.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              {count > 0 && (
                <span className={`ml-1.5 text-[10px] ${active ? "text-vodium-black/60" : "text-vodium-cream/25"}`}>
                  {count}
                </span>
              )}
            </button>
      ...
      <div className="bg-vodium-charcoal rounded-2xl border border-white/[0.06] overflow-hidden">
        ...
        {filtered.length === 0 && (
          <div className="px-6 py-20 text-center">
            ...
            {credits.length === 0 ? (
              <>
                <p className="text-vodium-cream/40 text-sm mb-1">No credits recorded yet.</p>
                <p className="text-vodium-cream/22 text-xs mb-6">Start tracking your first credit.</p>
                <Link
                  href="/dashboard/credit/new"
              ...
              <>
                <p className="text-vodium-cream/40 text-sm mb-1">No credits match your filter.</p>
                <button
        ...
        {filtered.map((c) => {
          ...
          const dueLabel  = dueDate.toLocaleDateString("en-NG", { month: "short", day: "numeric" });
          ...
          return (
            <div
              ...
              className={[
                ...
                isOverdue ? "bg-rose-500/[0.025] hover:bg-rose-500/[0.04]" : "hover:bg-white/[0.02]",
              ].join(" ")}
            >
              {/* Mobile layout */}
              <div className="md:hidden px-5 py-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-sm text-vodium-cream truncate">{c.student.fullName}</p>
                    {c.student.matricNumber && (
                      <p className="text-xs text-vodium-cream/35 mt-0.5">{c.student.matricNumber}</p>
                    )}
                  </div>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${cfg.className}`}>
                    {cfg.label}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-vodium-cream/35 mb-0.5">Amount</p>
                    <p className="font-serif text-base text-vodium-gold">{formatNaira(c.amount)}</p>
                  </div>
                  {c.amountRepaid > 0 && (
                    <div className="text-right">
                      <p className="text-xs text-vodium-cream/35 mb-0.5">Repaid</p>
                      <p className="font-serif text-base text-emerald-400">{formatNaira(c.amountRepaid)}</p>
                    </div>
                  )}
                  <div className="text-right">
                    <p className="text-xs text-vodium-cream/35 mb-0.5">Balance</p>
                    <p className={`font-serif text-base ${isPaid ? "text-vodium-cream/25" : isOverdue ? "text-rose-400" : "text-vodium-cream"}`}>
                      {formatNaira(balance)}
                    </p>
                  </div>
                </div>
              ...
              <div className="hidden md:grid grid-cols-12 items-center px-5 py-3.5 gap-2">
                ...
                <div className="col-span-3 flex items-center gap-2.5 min-w-0">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 text-[11px] font-bold ${isOverdue ? "bg-rose-500/15 text-rose-400" : "bg-vodium-gold/10 text-vodium-gold"}`}>
                    {c.student.fullName.charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm text-vodium-cream font-medium truncate leading-tight">{c.student.fullName}</p>
                    <p className="text-[10px] text-vodium-cream/30 mt-0.5">{c.student.matricNumber ?? "—"}</p>
                  </div>
                </div>

                {/* Note */}
                <div className="col-span-2">
                  <p className="text-xs text-vodium-cream/35 truncate">{c.description ?? "—"}</p>
                </div>

                {/* Amount */}
                <div className="col-span-2 text-right">
                  <p className="font-serif text-sm text-vodium-gold">{formatNaira(c.amount)}</p>
                </div>

                {/* Repaid */}
                <div className="col-span-2 text-right">
                  <p className={`text-sm font-medium ${c.amountRepaid > 0 ? "text-emerald-400" : "text-vodium-cream/20"}`}>
                ...
                <div className="col-span-1 text-center">
                  <p className={`text-[11px] ${isOverdue ? "text-rose-400" : "text-vodium-cream/35"}`}>{dueLabel}</p>
                </div>
